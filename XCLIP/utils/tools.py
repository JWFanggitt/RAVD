...
class AverageMeter:
    ...
    def sync(self):
        ...
        self.val = reduce_tensor(val, 1).item()
...
def generate_text(data):
    # text_aug = f"{{}}"
    classes = torch.cat([clip.tokenize(c, context_length=77) for c in data])
    # classes = torch.cat([clip.tokenize(text_aug.format(c), context_length=77) for c in data])
