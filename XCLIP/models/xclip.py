...
class XCLIP(CLIP):
    def __init__(self,
        ...
        self.token_embedding1 = nn.Embedding(vocab_size, 2*transformer_width)
        self.positional_embedding = nn.Parameter(torch.empty(self.context_length, transformer_width))
        self.ln_final = LayerNorm(transformer_width)
        self.text_projection = nn.Parameter(torch.empty(transformer_width, embed_dim))
        self.text_projection1=nn.Parameter(torch.empty(transformer_width,2*embed_dim))
        ...
        self.prompts_visual_proj1 = nn.Parameter(torch.randn(vision_width, 2*embed_dim))
    ...
    def encode_text(self, text):
        ...
        x = x[torch.arange(x.shape[0]), eos_indx] @ self.text_projection1
        x = x.reshape(K, -1)
        return x
    ...
    def encode_video(self, image):
        ...
        img_features = img_features @ self.prompts_visual_proj1
        
        cls_features = cls_features.view(b, t, -1)
        img_features = img_features.view(b,t,-1,1024)
