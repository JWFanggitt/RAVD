...
class ResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None):
        ...
        self.attn = nn.MultiheadAttention(2*d_model, n_head)
        self.ln_1 = nn.LayerNorm(2*d_model)
        self.mlp = nn.Sequential(OrderedDict([
            ("c_fc", nn.Linear(2*d_model, d_model * 4)),
            ("gelu", QuickGELU()),
            ("c_proj", nn.Linear(d_model * 4, 2*d_model))
        ]))
        self.ln_2 = nn.LayerNorm(2*d_model)
...
class MultiframeIntegrationTransformer(nn.Module):
    def __init__(self, T, embed_dim=512, layers=1,):
        ...
        self.positional_embedding = nn.Parameter(torch.empty(1, T, 2*embed_dim))
