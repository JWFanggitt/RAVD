...
class MulitHeadAttention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.q_proj = nn.Linear(2*dim, 2*dim, bias=qkv_bias)
        self.k_proj = nn.Linear(2*dim, 2*dim, bias=qkv_bias)
        self.v_proj = nn.Linear(2*dim, 2*dim, bias=qkv_bias)
...
class PromptGeneratorLayer(nn.Module):
    def __init__(
        ...
        self.norm3 = nn.LayerNorm(2*d_model)
...
class VideoSpecificPrompt(nn.Module):
    def __init__(self, layers=2, embed_dim=512, alpha=0.1,):
        ...
        self.alpha = nn.Parameter(torch.ones(2*embed_dim) * alpha)
