...
#         return loss
triplet_loss = torch.nn.TripletMarginLoss(margin=0.5,p=2.0,eps=1e-5,swap=False,reduction="mean")
import numpy as np
class NCELearnableTempLoss_vsc(nn.Module):
    """
    Compute contrastive loss: video-(sub,cap)
    ...
    # def forward(self, vis_feat1, text_feat1, vis_feat2, text_feat2, vis_feat3, text_feat4, vis_feat4, text_feat4, temp):
    def forward(self,clip1,clip2,clip3,clip4):
        ...
        logits2_pos = logits1[diag].reshape(logits2.shape[0], 1)
        logits3_pos = logits1[diag].reshape(logits3.shape[0], 1)
        logits4_pos = logits1[diag].reshape(logits4.shape[0], 1)
        logits1_neg = logits1[~diag].reshape(logits1.shape[0], -1)
        logits2_neg = logits1[~diag].reshape(logits1.shape[0], -1)
        logits3_neg = logits1[~diag].reshape(logits1.shape[0], -1)
        logits4_neg = logits1[~diag].reshape(logits1.shape[0], -1)
        # triple1=torch.cat([logits1_pos, logits3_pos, logits4_pos], dim=1)
        # triple2=torch.cat([logits2_pos, logits4_pos,logits1_pos],dim=1)
        l1=triplet_loss(logits1_pos, logits3_pos, logits4_pos)
        l2=triplet_loss(logits2_pos, logits4_pos,logits1_pos)
        # v2t = torch.cat([v2t_pos, v2t_neg, v2t_neg_2], dim=1)
        # v2t_2 = torch.cat([v2t_pos_2, v2t_neg, v2t_neg_2], dim=1)
        # v2t_label = torch.zeros(v2t.shape[0], dtype=torch.long).to(v2t.device)
        #
        # loss = F.cross_entropy(v2t, v2t_label) + F.cross_entropy(v2t_2, v2t_label).mean()
        return l1+l2
