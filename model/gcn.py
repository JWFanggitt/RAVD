...
class GraphTripleConv(nn.Module):
    ...
    def __init__(self, input_dim_obj, input_dim_pred, output_dim=None, hidden_dim=512,
        ...
        net1_layers1= [3456, hidden_dim, 2 * hidden_dim + output_dim]
        net1_layers = [l for l in net1_layers if l is not None]
        self.net1 = build_mlp(net1_layers, batch_norm=mlp_normalization)
        self.net1.apply(_init_weights)

        self.net1_1 = build_mlp(net1_layers1, batch_norm=mlp_normalization)
        self.net1_1.apply(_init_weights)
        ...
        if self.residual:
            ...
            self.linear_projection1 = nn.Linear(1152, output_dim)
            self.linear_projection_pred = nn.Linear(input_dim_pred, output_dim)
            self.linear_projection_pred1 = nn.Linear(1152, output_dim)
    ...
    def forward(self, obj_vecs, pred_vecs, edges):
        ...
        if cur_t_vecs.shape[1]==3456:
            new_t_vecs = self.net1_1(cur_t_vecs)
        else:
        ...
        if self.residual:
            if obj_vecs.shape[1]!=640:
                projected_obj_vecs = self.linear_projection1(obj_vecs)
            else:
                projected_obj_vecs = self.linear_projection(obj_vecs)
            new_obj_vecs = new_obj_vecs + projected_obj_vecs
            # new
            if pred_vecs.shape[1]!=640:
                new_p_vecs = new_p_vecs + self.linear_projection_pred1(pred_vecs)
            else:
...
def parse_args():
    ...
    parser.add_argument('--resolution', type=int, default=512, help='batch size')
    parser.add_argument('--dataloader_num_workers', type=int, default=4, help='batch size')
...
if __name__ == '__main__':
    gcn_encoder = {
        'input_dim_obj': 64 * 2 + 512,  # 64
        'input_dim_pred': 64 * 2 + 512,
        'hidden_dim': 64 * 4,  # gconv_dim * 4
        'pooling': 'avg',  #
        'num_layers': 5,  # 5
    ...
    clip_dim = 512
    embedding_dim = 64
    gcn_hidden_dim = embedding_dim * 4
    obj_embedding_dim = embedding_dim
    box_embedding_dim = embedding_dim

    gcn_encoder = GraphTripleConvNet(**gcn_encoder)
    mlp_mean_var = build_mlp(
        [embedding_dim * 2 + clip_dim, gcn_hidden_dim, embedding_dim * 2],
        batch_norm="batch",
