...
class GraphTripleConv(nn.Module):
    ...
    def __init__(self, input_dim_obj, input_dim_pred, output_dim=None, hidden_dim=512,
        ...
        net1_layers = [1792, hidden_dim, 2 * hidden_dim + output_dim]
        net1_layers = [l for l in net1_layers if l is not None]
        self.net1 = build_mlp(net1_layers, batch_norm=mlp_normalization)
        self.net1.apply(_init_weights)

        # net2_layers = [hidden_dim, hidden_dim, output_dim]
        net2_layers = [hidden_dim, hidden_dim, 576]
    ...
    def forward(self, obj_vecs, pred_vecs, edges,map_number):
        """
        Inputs:
        ...
        map_number=map_number.squeeze(1)
        m_s, m_p, m_o = map_number.chunk(3, dim=1)  # All have shape (T, 1)
        m_s, m_p, m_o = [x.squeeze(1) for x in [m_s, m_p, m_o]]  # Now have shape (T,)
        m_edges = torch.stack([m_s, m_p], dim=1)  # Shape is (T, 2)

        # s_idx = edges[:, 0].contiguous()
        # o_idx = edges[:, 1].contiguous()

        s_idx = m_edges[:, 0].contiguous()
        o_idx = m_edges[:, 1].contiguous()
...
class GraphTripleConvNet(nn.Module):
    ...
    def forward(self, obj_vecs, pred_vecs, edges,map_number):
        for i in range(self.num_layers):
            ...
            obj_vecs, pred_vecs = gconv(obj_vecs, pred_vecs, edges,map_number)
