...
class TextToVideoSDPipeline(DiffusionPipeline, TextualInversionLoaderMixin):
    ...
    def __init__(
            ...
            contronet: ControlUNet3DConditionModel,
    ...
    def __call__(
            ...
            control_depth_videos: Optional[torch.FloatTensor] = None,
            control_mask_videos: Optional[torch.FloatTensor] = None,
        ...
        if task_type=="predict":
            # for video prediction, we only condition on the first 6 frames and predict the next 10 frames.
            control_depth_videos[:, 6:16,:, :, :] = 0
            control_mask_videos[:, 6:16,:, :, :] = 0
        elif task_type=="backward":
            # for video prediction, we only condition on the last 6 frames and recovery the previous 10 frames.
             control_depth_videos[:, 0:10, :, :, :] = 0
             control_mask_videos[:, 0:10, :, :, :] = 0
        else:
             control_depth_videos= None
             control_mask_videos = None
        ...
        def _assert_finite(name, value):
            ...
            if not torch.isfinite(value).all():
                raise ValueError(
                    f"{name} contains NaN/Inf during task_type={task_type}"
                )
        ...
        prompt_embeds = self._encode_prompt1(
            prompt,
            device,
            num_images_per_prompt,
            do_classifier_free_guidance,
        ...
        if task_type=="predict":
            ...
            cond_latents = latents[:, :, :m, :, :]
            noise_latents[:, :, 0:m, :, :] = cond_latents
            for f in range(num_frames):
                if f < cond_frames:
                    base_ratio = 0.3
                    source_cond = cond_latents[:, :, f:f + 1, :, :]
                else:
                    decay_ratio = 1 - (f - cond_frames) / (num_frames - cond_frames)
                    base_ratio = 0.01* decay_ratio
                    source_cond = cond_latents[:, :, pivot_frame:pivot_frame + 1, :, :]
                noise_latents[:, :, f:f + 1, :, :] = (
                        0.05 * source_cond +
                        (1 - base_ratio) * noise_latents[:, :, f:f + 1, :, :]
                )
            noise_latents[:, :, 0:m, :, :] = cond_latents
            ...
            for f in range(num_frames):
                if f >= num_frames - cond_frames:
                    ...
                    base_ratio = 0.3
                    source_cond = cond_latents[:, :, f - (num_frames - cond_frames):f - (num_frames - cond_frames) + 1, :,:]
                else:
                   
                    decay_ratio = (num_frames - cond_frames - f) / (num_frames - cond_frames)
                    base_ratio = 0.01 * decay_ratio
                    source_cond = cond_latents[:, :, pivot_frame - 1:pivot_frame, :, :]
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                control_depthss = torch.cat(
                    [control_depth_videos] * 2) if do_classifier_free_guidance else control_depth_videos
                
                control_maskss = torch.cat(
                    [control_mask_videos] * 2) if do_classifier_free_guidance else control_mask_videos
               

                down_block_res_samples, mid_block_res_sample = self.contronet(
                    latent_model_input, t, encoder_hidden_states=prompt_embeds,
                    controlnet_down_cond=control_depthss, controlnet_up_cond=control_maskss,
                )
                ...
                _assert_finite("noise_pred", noise_pred)
                ...
                if task_type=="predict":
                    latents[6:16, :, :, :] = self.scheduler.step(noise_pred[6:16, :, :, :], t, latents[6:16, :, :, :],
                                                                **extra_step_kwargs).prev_sample
                
                
                elif task_type=="backward":
                    latents[0:10, :, :, :] = self.scheduler.step(noise_pred[0:10, :, :, :], t, latents[0:10, :, :, :],
                                                                 **extra_step_kwargs).prev_sample

                  
                # latents[1:16, :, :, :] = self.scheduler.step(noise_pred[1:16, :, :, :], t, latents[1:16, :, :, :],
                #                                              **extra_step_kwargs).prev_sample
